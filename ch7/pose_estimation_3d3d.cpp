...
// 二元边  ，使用二元边父类
class EdgeProjectXYZRGBDPoseSpacePoint: public g2o::BaseBinaryEdge<3, Eigen::Vector3d, g2o::VertexSBAPointXYZ, g2o::VertexSE3Expmap>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    // EdgeProjectXYZRGBDPoseOnly(const Eigen::Vector3d& point): _point(point){}
    EdgeProjectXYZRGBDPoseSpacePoint(){}
...
void bundleAdjustmentWithSpacePoint(const vector<Point3f>& pts1, 
    ...
    int index = 0;
    for(size_t i = 0; i < pts1.size(); i++){
        EdgeProjectXYZRGBDPoseSpacePoint* edge = new EdgeProjectXYZRGBDPoseSpacePoint();
        edge->setId(index);
        // 二元边不能自己连接自己
...
int main ( int argc, char** argv ){
    ...
    bundleAdjustmentWithSpacePoint(pts_1, pts_2, R, t);
